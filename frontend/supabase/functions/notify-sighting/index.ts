...
// Edge Function: notify-sighting
// Triggered via Postgres trigger (net.http_post) on sightings inserts.
// Sends an email to the item owner via Brevo API.
...
async function sendEmail(payload: { to: string; subject: string; text: string; html: string }) {
  ...
  if (!smtpKey || !senderEmail) {
    console.error("[notify-sighting] Missing Brevo env vars");
  ...
  if (!response.ok) {
    ...
    console.error("[notify-sighting] Brevo API error:", error);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      console.error("[notify-sighting] Missing Supabase env vars");
    ...
    if (itemError || !item?.owner_id) {
      console.error("[notify-sighting] Item fetch error", itemError);
    ...
    // owner_id pode ser text; converter se necessário
    ...
    if (ownerError || !ownerProfile?.email) {
      console.error("[notify-sighting] Owner fetch error", ownerError);
...
<p><a href="${sightingPage}" target="_blank" rel="noopener noreferrer" style="display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 4px;">Ver detalhes</a></p>`;
    ...
    console.error("[notify-sighting] Error", err);
